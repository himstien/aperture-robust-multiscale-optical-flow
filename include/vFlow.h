...
private:    
	...
	EventMatrix<double> lastFlowTime;// = new EventMatrix<double> (width, height, 0);    
...
public:
    ...
    bool run(); //run over all file
    bool run(int); // run over given number of events only
    void close(); // close the program and clear memory. *not defined yet*

	EventMatrix<double> returnFlowTime(){return lastFlowTime; }; //[DEBUG] test function to check if arrays are initialized correctly.
